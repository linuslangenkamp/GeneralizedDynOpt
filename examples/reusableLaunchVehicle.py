from gdopt import *


model = Model("reusableLaunchVehicle")

pi = 3.141592653589793238462643383279 

cft2m = 0.3048
cft2km = cft2m / 1000
cslug2kg = 14.5939029

Re = 20902900*cft2m
S = 2690*cft2m**2
cl1 = -0.2070
cl2 = 1.6756
cd1 = 0.0785
cd2 = -0.3529
cd3 = 2.04
b1 = 0.07854
b2 = -0.061592
b3 = 0.00621408
H = 23800*cft2m
al1 = -0.20704
al2 = 0.029244
rho0 = 0.002378*cslug2kg/cft2m**3
mu = 1.4076539e16*cft2m**3
mass = 6309.433*cslug2kg

alt0 = 260000*cft2m;
rad0 = alt0+Re;
lon0 = 0;
lat0 = 0;
speed0 = 25600*cft2m;
fpa0 = -1*pi/180;
azi0 = 90*pi/180;
altf = 80000*cft2m;
radf = altf+Re;
speedf = 2500*cft2m;
fpaf = -5*pi/180;
azif = -90*pi/180;

rad = model.addState(start=rad0, lb=Re, ub=rad0, symbol="height")
lon = model.addState(start=lon0, lb=-pi, ub=pi, symbol="longitude")
lat = model.addState(start=lat0, lb=-70 * pi / 180, ub=70*pi/180, symbol="latitude")
speed = model.addState(start=speed0, lb=10, ub=45000, symbol="speed")
fpa = model.addState(start=fpa0, lb=-80*pi/180, ub=80*pi/180, symbol="flightpath")
azi = model.addState(start=azi0, lb=-pi, ub=pi, symbol="azimuth")

aoa = model.addControl(lb=-pi/2, ub=pi/2, guess=17 * pi/180, symbol="angleOfAttack")
bank = model.addControl(lb=-pi/2, ub=1*pi/180, guess=-pi/2 * 0.8, symbol="bankAngle")

altitude = rad - Re

CD = cd1+cd2*aoa+cd3*aoa**2
rho = rho0*exp(-altitude/H)
CL = cl1+cl2*aoa
gravity = mu/rad**2
dynamic_pressure = 0.5*rho*speed**2
D = dynamic_pressure*S*CD/mass
L = dynamic_pressure*S*CL/mass
slon = sin(lon)
clon = cos(lon)
slat = sin(lat)
clat = cos(lat)
tlat = tan(lat)
sfpa = sin(fpa)
cfpa = cos(fpa)
sazi = sin(azi)
cazi = cos(azi)
cbank = cos(bank)
sbank = sin(bank)

model.addF(rad, speed*sfpa, nominal=rad0)
model.addF(lon, speed*sfpa*sazi / (rad * clat))
model.addF(lat, speed * cfpa * cazi / rad)
model.addF(speed, -D-gravity*sfpa, nominal=rad0)
model.addF(fpa, (L * cbank-cfpa*(gravity-speed**2/rad))/speed)
model.addF(azi, (L * sbank /cfpa + speed**2 * cfpa * sazi * tlat / rad) / speed)

model.addFinal(rad, eq=radf)
model.addFinal(speed, eq=speedf)
model.addFinal(fpa, eq=fpaf)

model.addMayer(-lat)

model.setTolerance(1e-10)
model.setMeshIterations(15)
model.setMeshLevel(1.3)
model.setMeshAlgorithm(MeshAlgorithm.L2_BOUNDARY_NORM)
model.setMuStrategyRefinement(MuStrategy.MONOTONE)
model.setMuInitRefinement(1e-14)

model.solve(tf=2009.35, steps=10, rksteps=4)
model.plot(dots=Dots.BASE)